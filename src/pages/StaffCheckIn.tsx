...
export default function StaffCheckIn() {
  ...
  const handleCheckReservation = async (plateOverride?: string) => {
    const targetPlate = plateOverride || searchPlate;
    if (!targetPlate.trim()) {
      showToast('Vui lòng nhập hoặc quét biển số xe để tìm kiếm đặt trước.', 'error');
      return;
    }
    ...
    try {
      const resList = await parkingService.getReservationsByKeyword(targetPlate.trim().toUpperCase());
    ...
    <div className="min-h-screen bg-slate-50 text-slate-700 font-sans antialiased flex flex-col relative">
                ...
                <div>
                  ...
                  <div className="flex space-x-2">
                    ...
                    <button
                      ...
                      className="px-3 bg-slate-100 hover:bg-slate-250 border border-slate-250 text-slate-700 text-xs font-bold rounded-xl cursor-pointer transition flex items-center space-x-1"
                    >
      ...
      <QrScannerModal
        ...
        onScanSuccess={(decodedText) => {
          setSearchPlate(decodedText.toUpperCase());
          handleCheckReservation(decodedText.toUpperCase());
        }}
      />
    </div>
  );
